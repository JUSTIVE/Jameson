...
let rec keySet (parentPath: string) (state: Set<string>) (jsonValue: JsonValue) : FileKeySet =
    let setJoinToSeq (x: seq<'a>) (y: Set<'a>) : seq<'a> = x |> Seq.append (y |> Set.toSeq)
    ...
    | JsonValue.Record properties ->
        ...
        |> Array.fold setJoinToSeq Seq.empty
        |> Set.ofSeq
    | x -> Set.add (PseudoJson.make parentPath x) Set.empty

let parse (jsonValue: JsonValue) : FileKeySet = keySet "" Set.empty jsonValue
